...
namespace cg::renderer
{
    ...
    protected:
        ...
        // Updated declarations to match the implementation:
        std::filesystem::path get_shader_path(const std::string& shader_name);
        ComPtr<ID3DBlob> compile_shader(const std::filesystem::path& shader_path, const std::string& entrypoint, const std::string& target);
